...
1st exercise:
Control Props are objectively more powerful because they allow complete control over state from outside the component. We are controlling the state of Toggle component from another component, App, and a hook, useToggle.

State reducers do not have to manage the component's state themselves (though they can manage some of their own state as needed). They just enhance the capabilities of reducers that manage the state of the component. The biggest limitation of a state reducer is that it's impossible to set state of the component from outside it's normal setState calls.

On managing the state on our own:

the dispatch function in the hook
...
When we pass undefined state to input and then setState to be some truthy value, we change it from uncontrolled to be controlled.
When we pass some state of a truthy value but update it to be undefined, we change it from controlled to be uncontrolled.

Runtime warnings are helpful during development, but probably not useful in production. 
process.env.NODE_ENV === 'production'

App defines the state for all inputs. It passes this state to Toggle + Toggle gets an event handler (onChange) defined in the App. Toggle calls useToggle hook and gets a prop getter that sets up props for Toggle children, but at the same time Toggle passes useToggle an event handler (onChange). useToggle based on the event process some logic and sends back the necessary state to the handler (when it calls the handler on the event) in the App so that could use that state for some logic. So overall, we could set the state outside the Toggle component.
