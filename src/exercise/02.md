...
Interestingly, the React docs says that we shouldn't use Children API
as we have multiple options available to avoid that.

I understand the perspective of Kent on using the compound components,
but at the same time understand the perspective of those who write React docs. So I would say I should be just aware of these approaches and use them accordingly on case by case basis.

Check the js doc for the exercise, the approach gives us flexibility that isn't mentioned in the documentation where we can restrict components we want to pass props to (implicit state)
