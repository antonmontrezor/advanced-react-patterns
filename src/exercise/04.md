...
Prop Collections: The whole idea is around identifying what's the common use case for the hook or/and component and return just the necessary props for that use case in a single object. This way, users can spread out that object across multiple elements and use the needed properties of that object.

Prop getters:
It's a nice pattern that is more flexible and powerful that prop collections as we can configure both the default props we are setting and the customized ones
